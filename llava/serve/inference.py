...
# Drawing functions from the first code
...
def main(args):
    ...
    for entry_id, json_responses in ret.items():
        for json_response in json_responses:
            img = load_image(os.path.join(args.data_path, [d['image'] for d in data if d['id'] == entry_id][0]))
            drawn_img = draw_boxmap(json_response, img, CLS2COLOR["QB-Poster"])  # Adjust the category as needed
            drawn_img.save(os.path.join(args.data_path, f"{entry_id}_boxed.jpg"))

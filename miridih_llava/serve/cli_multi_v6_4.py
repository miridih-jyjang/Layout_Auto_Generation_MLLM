...
def draw_box(img, elems, cls2color, data_path):
    ...
    for file_name, clss, box, layer in elems:
        template_id, page_num, ele_num = file_name.split('/')[-1].split('.')[0].split('_')
        page_num = int(page_num)
        image_file = f"miridih-v6.4/images/{template_id:08}/{page_num:03}/{template_id:08}_{page_num:01}_{ele_num:01}.png"
...
def online_rendering(image_folder, skin_img_file, annotations, i_entry, args, task):
    ...
    for idx, anno in enumerate(annotations):
        ...
        template_id, page_num, ele_num= ele_img_file.split('/')[-1].split('_')[0]
        page_num = int(page_num)
        template_id = int(template_id)
        ele_num = int(ele_num)
        
        image_file = f"{image_folder}/miridih-v6.4/images/{template_id:08}/{page_num:03}/{template_id:08}_{page_num:01}_{ele_num:01}.png"
        overlay_img = Image.open(image_file).convert("RGBA")
    ...
    for i_entry, entry in enumerate(tqdm(data, desc="Processing entries")):
        ...
        if args.debug or args.image_out:
            ...
            if not os.path.isfile(os.path.join('/'.join(args.output_file.split('/')[:-1]), f"gt/{template_id}_{page_num:01}.jpg")):
                try:
                    thumbnail_image_file = image_file
            ...
            if "posterllava/posterllava_v0" in args.model_path:
                ...
                drawn_img = draw_boxmap(ret[entry['id']][-1], valid_filenames, invalid_filenames, image, CLS2COLOR["Crello"], args.data_path)  # Adjust the category as needed

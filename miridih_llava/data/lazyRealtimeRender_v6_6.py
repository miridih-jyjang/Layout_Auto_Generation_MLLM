...
class LazyRealTimeRenderingDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            for element in valid_filenames:
                element_file = element['file_name']
                template_id, page_num, ele_num = element['file_name'].split('.')[0].split('_')
                template_id = int(template_id)
                page_num = int(page_num)
                ele_num = int(ele_num)
                ele_file_path = os.path.join(self.data_args.image_folder, 'miridih-v6.6/images', f"{template_id:08}/{page_num:03}/{element_file}")
                ...
                if self.data_args.image_aspect_ratio == 'pad':
                    def expand2square(pil_img, background_color):
                        ...
                        if width == height:
                            ...
                            return result
                    ele_img = expand2square(ele_img, tuple(int(x*255) for x in processor.image_mean))
                    ele_img = processor.preprocess(ele_img, return_tensors='pt')['pixel_values'][0]
                else:
                    ele_img = processor.preprocess(ele_img, return_tensors='pt')['pixel_values'][0]
                pixel_values.append(ele_img.unsqueeze(0))  
            
            template_id, page_num = self.list_data_dict[i]['id'].split('_')
            template_id = int(template_id)
            page_num = int(page_num)
            image_folder = self.data_args.image_folder
            if os.path.isfile(f"{image_folder}/miridih-v6.6/images/{template_id:08}/{page_num:03}/{template_id:08}_{page_num:01}_1.png"):
                image_file = f"{image_folder}/miridih-v6.6/images/{template_id:08}/{page_num:03}/{template_id:08}_{page_num:01}_1.png"
            else:
                image_file = f"{image_folder}/miridih-v6.6/images/{template_id:08}/{page_num:03}/{template_id:08}_{page_num:01}_0.png"
                
            if self.list_data_dict[i]['image'] == "complete" or self.list_data_dict[i]['image'] == "refine":
    ...
    def modify_rects(self, input_string):
        ...
        for full_match, label, box in rects:
            ...
            new_w = process_float(box[2]) if box[0] != None else "''"
            new_h = process_float(box[3]) if box[0] != None else "''"

            # Reconstruct the rect element with the modified values
            new_rect = (f"'label': '{label}', 'box': [{new_x1}, {new_y1}, {new_w}, {new_h}]")
    ...
    def online_rendering(self, image_folder, image_file, annotations):
        ...
        for idx, anno in enumerate(annotations):
            ...
            page_num = int(page_num)
            image_file = f"{image_folder}/miridih-v6.6/images/{template_id:08}/{page_num:03}/{ele_img_file}"
            overlay_img = Image.open(image_file).convert("RGBA")
            ele_width, ele_height = W*(float(anno['w'])-float(anno['x1'])), H*(float(anno['h']) - float(anno['y1']))
...
if __name__ == "__main__":
    ...
    data_path = "/workspace/data/miridih-v6.6/annotations/train_llava_numerical.json"

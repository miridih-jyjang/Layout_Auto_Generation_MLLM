...
class LazyRealTimeRenderingDataset(Dataset):
    ...
    def __init__(self, data_path: str, ele_cache_path: str,
        ...
        self.bbox_layer_IMG_extract = re.compile(r"'label':\s*'([^']+)',\s*'box':\s*\[([-\d.,\s]*)\],\s*'layer':\s*(\d+),\s*'\<image(\d+)\>':\s*'<image>',\s*'file_name':\s*'([^']*)'")
                        ...
                        if '<image>' in text:
                            if 'layer' in filename_dict:
                                ...
                                removal_pattern = (r"\{'label':\s*'" + re.escape(label) +
                                    ...
                                    r"|\d+),\s*\<image\d+\>:\s*'<image>',\s*'file_name':\s*'" + re.escape(file_name) +
                                ...
                                removal_pattern = (r"\{'label':\s*'" + re.escape(label) +
                                    ...
                                    re.escape(format(y1, '.4f')) + r"|''|\d*\.\d+),\s*(" +
                                    re.escape(format(x2, '.4f')) + r"|''|\d*\.\d+),\s*(" +
                                    re.escape(format(y2, '.4f')) + r"|''|\d*\.\d+)\],\s*\<image\d+\>:\s*'<image>',\s*'file_name':\s*'" + re.escape(file_name) +
                                    r"',\s*'src':\s*'" + re.escape(src) + r"'\}")
                        ...
                        if "'<image>'," in text:
                            if 'layer' in filename_dict:
                                ...
                                removal_pattern = (r"\{'label':\s*'" + re.escape(label) +
                                    ...
                                    re.escape(layer) + r"|\d*|''),\s*'\<image\d+\>':\s*'<image>',\s*'file_name':\s*'" + 
                                    re.escape(file_name) + r"'\}")
                            else:
                                removal_pattern = (r"\{'label':\s*'" + re.escape(label) +
                                    r"',\s*'box':\s*\[\s*(''|[-\d.]+),\s*(''|[-\d.]+),\s*(''|[-\d.]+),\s*(''|[-\d.]+)\],\s*'\<image\d+\>':\s*'<image>',\s*'file_name':\s*'" + 
                                    re.escape(file_name) + r"'\}")
...
if __name__ == "__main__":
    ...
    data_path = "/workspace/data/crello-v6.7/annotations_1/val_llava_numerical.json"
    ...
    dataset = LazyRealTimeRenderingDataset(data_path, ele_cache_path, tokenizer, data_args)

...
class CustomDataLoader(DataLoader):  
        ...
        if coord_lst is not None and category_lst is not None: # create the training data   
            for coords, categories, filenames, priorities in zip(coord_lst, category_lst, filename_lst, priority_lst):
                ...
                for coord, category, file_name, priority in zip(coords, categories, filenames, priorities):
                    ...
                    ele_dict = {"label": real_category, "src": f"[IMG{i+1}]", "x1": round(x1/W, 4), "y1":round(y1/H, 4), "w":round(w/W, 4), "h":round(h/H, 2), "z":priority, "file_name": file_name}                    # ele_dict = {"label": real_category, "x": x, "y":y, "w":w, "h":h, "content":content}
    ...
    def custom_function(self, data, id_, self_consistency=True, consistency_num=10, min_max=[-1,-1]): 
        ...
        cond_recover_mask = [
            self.cond_random_mask.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in cond_recover_mask
        ]
        unconditional = [
            self.unconditional.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in unconditional
        ]
        cond_cate_to_size_pos = [
            self.cond_cate_to_size_pos.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in cond_cate_to_size_pos
        ]
        cond_cate_size_to_pos = [
            self.cond_cate_size_to_pos.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in cond_cate_size_to_pos
        ]
        cond_cate_pos_to_size = [
            self.cond_cate_pos_to_size.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in cond_cate_pos_to_size
        ]
        completion = [
            self.completion.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in completion
        ]
        refinement = [
            self.refinement.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in refinement
        ]
        coord_pred = [
            self.coord_pred.format(num_ele=num_ele, resolution="[{},{}]".format(W.item(),H.item()), template_type=template_type[0], bbox_json=bbox)
            for bbox in coord_pred
        ]
        ...
        if self.split == "train" or self.split == "val":
            ...
            if self.infilling:
                ...
                coord_pred_seq_modeling = [
                    self.glue_template_codegen_test.format(instruct=sample)
                    for sample in coord_pred
                ]

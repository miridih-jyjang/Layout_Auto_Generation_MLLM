...
def is_valid_bbox(bbox, x_max, y_max):
        return 0 <= bbox["x1"] < 1 and 0 <= bbox["x2"] < 1  and 0 <= bbox["y1"] < 1 and 0 <= bbox["y2"] < 1
...
def add_gaussian_noise_and_resample(ele, x_max, y_max, sigma=0.05):
    ...
    def add_gaussian_noise(bbox):
        ...
        noise_w = np.random.normal(0, (bbox["x2"] - bbox["x1"])/4)
        noise_h = np.random.normal(0, (bbox["y2"] - bbox["y1"])/4)
...
class CustomDataLoader(DataLoader):  
    ...
    def __init__(
        ...
        **kwargs
        ...
        if args.add_task_instruction:
            ...
            task_instruction = TASK_INSTRUCTION[args.dataset_name.replace("crello_v6.5", "crello_v6.5_coord_pred")]
    ...
    def build_input_with_ele_dict(self, ele_dict: Dict, type=None):
        ...
        if type == "html_content":
            ...
            answer_notepad = [ele_dict["x2"],ele_dict["y2"]]
            ele_dict["x2"] = PLACE_HOLDER
            ele_dict["y2"] = PLACE_HOLDER
        ...
        if coord_lst is not None and category_lst is not None: # create the training data   
            for coords, categories, filenames, priorities in zip(coord_lst, category_lst, filename_lst, priority_lst):
                ...
                for coord, category, file_name, priority in zip(coords, categories, filenames, priorities):
                    ...
                    ele_dict = {"label": real_category, "src": f"[IMG{i+1}]", "x1": round(x1/W, 4), "y1":round(y1/H, 4), "x2":round(x2/W, 4), "y2":round(y2/H, 2), "z":priority, "file_name": file_name}                    # ele_dict = {"label": real_category, "x": x, "y":y, "w":w, "h":h, "content":content}

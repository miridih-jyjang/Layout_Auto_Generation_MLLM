...
def is_valid_bbox(bbox, x_max, y_max):
        # return 0 <= bbox["x1"] < 1 and 0 <= bbox["w"] < x_max  and 0 <= bbox["y1"] < 1 and 0 <= bbox["h"] < y_max
        return 1
    
def add_gaussian_noise_and_resample(ele, x_max, y_max, sigma=0.25, d=4):
    bbox = copy.deepcopy(ele)
    if not is_valid_bbox(ele, x_max, y_max):
        return ele
    def add_gaussian_noise(bbox, d):
        noise_x = np.random.normal(0, sigma)
        noise_y = np.random.normal(0, sigma)
        noise_w = np.random.normal(0, (bbox["w"])/4)
        noise_h = np.random.normal(0, (bbox["h"])/4)
        bbox['x1'] = max(round(bbox["x1"]+noise_x, d), 0.0)
        bbox['y1'] = max(round(bbox["y1"]+noise_y, d), 0.0)
        bbox['w'] = min(round(bbox["w"]+noise_w, d), 1.0)
        bbox['h'] = min(round(bbox["h"]+noise_h, d), 1.0)
        
        return bbox
    

    bbox = add_gaussian_noise(bbox, d)
...
class CustomDataLoader(DataLoader):  
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.json_template = TEMPLATE_FORMAT.get("json_target")
        self.bbox_template = TEMPLATE_FORMAT.get("json_bbox_format")
        self.W=0
        self.H=0
        ...
        if args.infilling:
            ...
            self.cond_cate_to_size_pos = INSTRUCTION_JSON.get("cond_cate_to_size_pos")
            self.cond_cate_size_to_pos = INSTRUCTION_JSON.get("cond_cate_size_to_pos")
            self.cond_cate_pos_to_size = INSTRUCTION_JSON.get("cond_cate_pos_to_size")
            self.cond_random_mask = INSTRUCTION_JSON.get("cond_random_mask")
            self.unconditional = INSTRUCTION_JSON.get("unconditional")
            self.refinement = INSTRUCTION_JSON.get("refinement")
            self.completion = INSTRUCTION_JSON.get("completion")
    ...
    def build_input_with_ele_dict(self, ele_dict: Dict, type=None):
        ...
        if type == "html_content":
            ...
            answer_notepad = ele_dict["label"]
            ele_dict["label"] = PLACE_HOLDER
        elif type == "size_pos_mask_html":
            c = ele_dict["label"]
            file_name = ele_dict["file_name"]
            answer_notepad = [ele_dict[k] for k in ele_dict if k != "label"]
            ...
            answer_notepad = [ele_dict["x1"], ele_dict["y1"]]
            ...
            selected_mask_element = random.sample(["x1", "y1", "w", "h"], random_mask_num)
            answer_notepad = []
            for key in selected_mask_element:
                answer_notepad.append(ele_dict[key])
                ele_dict[key] = PLACE_HOLDER
        elif type == "refinement_html":
            ele_dict = add_gaussian_noise_and_resample(ele_dict,self.W,self.H, d=self.d)
        elif type == "coord_pred":
        ...
        if type != "html_content":
            img_idx = ele_dict['src']
            result_string = f"{{'label': '{label}', 'box': [{', '.join(formatted_box)}], 'layer': {ele_dict['z']}, '{img_idx}': '<image>', 'file_name': '{file_name}'}}"
        else:
            img_idx = ele_dict['src']
            answer_notepad = f"{{'label': '{label}', 'box': [{', '.join(formatted_box)}], 'layer': {ele_dict['z']}, '{img_idx}': '<image>', 'file_name': '{file_name}'}}"
    ...
    def convert_num_to_html(self, coord_lst=None, category_lst=None, filename_lst=None, priority_lst=None, self_consistency=False, consistency_num=10, canvas=[-1, -1]):
        ...
        unconditional, refinement, random_mask, completion, coord_pred = [""], [], [], [], []
    ...
    def custom_function(self, data, id_, self_consistency=True, consistency_num=10, min_max=[-1,-1]): 
        ...
        if self.bbox_quantization == "code":
            ...
            real_idx = round_nested_list(real_idx, self.d)
            ...
            coord_pred = preposed_res.get("coord_pred")
            coord_pred_ans = preposed_res.get("codegen_ans").get("coord_pred_ans")
        ...
        batched_cond_bbox = [
            self.json_template.format(bbox_json_list=batched_cond_bbox[i])
            for i in range(len(batched_cond_bbox))                 
        ]
        cond_cate_to_size_pos = [
            self.json_template.format(bbox_json_list=cond_cate_to_size_pos[i])
            for i in range(len(cond_cate_to_size_pos))  
        ]
        cond_cate_size_to_pos = [
            self.json_template.format(bbox_json_list=cond_cate_size_to_pos[i])
            for i in range(len(cond_cate_size_to_pos))  
        ]
        cond_cate_pos_to_size = [
            self.json_template.format(bbox_json_list=cond_cate_pos_to_size[i])
            for i in range(len(cond_cate_pos_to_size))
        ]
        unconditional = [
            self.json_template.format(bbox_json_list=unconditional[i])
            for i in range(len(unconditional))
        ]
        cond_recover_mask = [
            self.json_template.format(bbox_json_list=random_mask[i])
            for i in range(len(random_mask))  
        ]
        completion = [
            self.json_template.format(bbox_json_list=completion[i])
            for i in range(len(completion))
        ]
        refinement = [
            self.json_template.format(bbox_json_list=refinement[i])
            for i in range(len(refinement))
        ]
        coord_pred = [
            self.json_template.format(bbox_json_list=coord_pred[i])
            for i in range(len(coord_pred))
        ]
        
        # add task instructions make html format.
        cond_recover_mask = [
            self.cond_random_mask.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(cond_recover_mask)
        ]
        unconditional = [
            self.unconditional.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(unconditional)
        ]
        cond_cate_to_size_pos = [
            self.cond_cate_to_size_pos.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(cond_cate_to_size_pos) 
        ]
        cond_cate_size_to_pos = [
            self.cond_cate_size_to_pos.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(cond_cate_size_to_pos) 
        ]
        cond_cate_pos_to_size = [
            self.cond_cate_pos_to_size.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(cond_cate_pos_to_size) 
        ]
        completion = [
            self.completion.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(completion) 
        ]
        refinement = [
            self.refinement.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(refinement) 
        ]
        coord_pred = [
            self.coord_pred.format(num_ele=num_ele, resolution="[{},{}]".format(W[i],H[i]), template_type=template_type[0], bbox_json=bbox)
            for i, bbox in enumerate(coord_pred) 
        ]
        ...
        if self.split == "train" or self.split == "val":
            if self.infilling:
                ...
                target_seqs = [
                    self.json_template.format(bbox_json_list=batched_html_lst[i])
                ...
                target_seqs = [
                    self.json_template.format(bbox_json_list=completion_ans[i])
                    for i in range(W.size(0))
                ]
                ...
                coord_pred_seq_modeling = [
                    self.glue_template_codegen_train.format(instruct=sample, result=target)
                    for sample, target in zip(coord_pred, coord_pred_ans)
                ]
                ...
                return {
                    ...
                    "coord_pred_seq_modeling" : coord_pred_seq_modeling,
                    "name" : name #s
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_ele_num", type=int, default=1e5)
    parser.add_argument("--min_ele_num", type=int, default=0)
    parser.add_argument("--min_aspect_ratio", type=float, default=1e5)
    parser.add_argument("--canvas_aspect_ratio", type=float, default=1e5)
    parser.add_argument("--min_size", type=int, nargs=2, default=[-1, -1], help="Minimum width and height.")

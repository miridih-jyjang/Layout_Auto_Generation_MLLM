...
def add_gaussian_noise_and_resample(ele, x_max, y_max, sigma=0.25):
    ...
    def add_gaussian_noise(bbox):
        ...
        bbox['x1'] = max(round(bbox["x1"]+noise_x, 4), 0.0)
        bbox['y1'] = max(round(bbox["y1"]+noise_y, 4), 0.0)
        bbox['w'] = min(round(bbox["w"]+noise_w, 4), 1.0)
        bbox['h'] = min(round(bbox["h"]+noise_h, 4), 1.0)
        ...
        if coord_lst is not None and category_lst is not None: # create the training data   
            for coords, categories, filenames, priorities in zip(coord_lst, category_lst, filename_lst, priority_lst):
                ...
                for coord, category, file_name, priority in zip(coords, categories, filenames, priorities):
                    ...
                    ele_dict = {"label": real_category, "src": f"<image{i+1}>", "x1": round(x1/W, 4), "y1":round(y1/H, 4), "w":round(w/W, 4), "h":round(h/H, 2), "z":priority, "file_name": file_name}                    # ele_dict = {"label": real_category, "x": x, "y":y, "w":w, "h":h, "content":content}
